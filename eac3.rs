...
fn decode<'t>(&mut self, input: &'t [u8]) -> Option<Self::Output<'t>> {
	...
	let channel_mode = s.bits(3) as usize;
	let lfe = s.bit();
	let _bit_rate = 8 * frame_size * sample_rate / (blocks_len as u32 * 256);
	#[derive(Clone,Copy,Debug)] enum ChannelLayout { DualMono, Mono, Stereo, _3F, _2F1R, _3F1R, _2F2R, _3F2R } use ChannelLayout::*;
	let (full_bandwidth_channels, layout) = [(2,DualMono), (1,Mono), (2,Stereo), (3,_3F), (3,_2F1R), (4,_3F1R), (4,_2F2R), (5,_3F2R)][channel_mode];
	let all_channels = full_bandwidth_channels + if lfe { 1 } else { 0 };
	s.skip(5);
	assert!(!matches!(layout, DualMono));
    ...
    if s.bit() {
        assert!(!matches!(layout, DualMono|Mono|Stereo));
        let _preferred_downmix = s.bits(2);
        assert!(matches!(layout, _3F|_3F1R|_3F2R));
        let _center_mix_level_ltrt = s.bits(3);
		let _center_mix_level = s.bits(3);
		assert!(matches!(layout, _2F1R|_3F1R|_2F2R|_3F2R));
        let surround_mix_level_ltrt = s.bits(3); assert!(3 <= surround_mix_level_ltrt && surround_mix_level_ltrt <= 7);
		let surround_mix_level = s.bits(3); assert!(3 <= surround_mix_level && surround_mix_level <= 7);
		assert!(lfe);
        if s.bit() { let _lfe_mix_level = s.bits(5); }
		assert!(frame_type == Type::Independent as _);
		assert!(channel_mode > 0);
	    if s.bit() { s.skip(6) }
        if s.bit() { s.skip(6) }
        match s.bits(2) {
			0 => {},
	...
	fn map<T,U,const N: usize>(a : &ArrayVec::<T, N>, f: impl FnMut(&T)->U) -> ArrayVec::<U, 6> { a.iter().map(f).collect() }
	...
	fn map_channels<T,U>((coupling, channels):&Channels<T>, mut f: impl Fn(&T)->U) -> Channels<U> { (coupling.as_ref().map(&mut f), channels.iter().map(f).collect())}
    let mut exponent : Blocks<_> = if exponent {
		...
		let frame_exponent_strategy_combinations = [
			...
			[3,1,0,0,0,0],[3,1,0,0,0,3],[3,2,0,0,2,0],[3,2,0,0,3,3],[3,2,0,2,0,0],[3,2,0,2,0,3],[3,2,0,3,2,0],[3,2,0,3,3,3],[3,3,1,0,0,0],[3,3,2,0,0,3],[3,3,2,0,2,0],[3,3,2,0,3,3],[3,3,3,2,0,0],[3,3,3,2,0,3],[3,3,3,3,2,0],[3,3,3,3,3,3],
		];
    ...
    let _aht = aht.then(|| {
		...
		map_channels(&channels, |e:&Blocks<_>| e.iter().all(|&e| e.map(|e| e==0).unwrap_or(false)))
	});
	/*let aht = aht.then(|| (coupling.iter().all(|&x| x).then(|| exponent.iter().all(|e| e.0.unwrap() == 0) && s.bit()),
		(0..all_channels).map(|channel| exponent.iter().all(|e| e.1[channel] == 0) && s.bit()).collect()));*/
	...
	unimplemented!("ac3_decode_frame parse_frame_header")
}}