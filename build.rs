fn main() {
    ...
    bindgen::builder().header("va.h").allowlist_function("va.*").parse_callbacks(Box::new(bindgen::CargoCallbacks)).derive_default(true).layout_tests(false).generate_comments(false).generate().unwrap()
