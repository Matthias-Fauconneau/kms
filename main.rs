...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let con = coninfo.iter().find(|&i| i.state() == connector::State::Connected).expect("No connected connectors");
    ...
    if card.atomic_commit(AtomicCommitFlags::ALLOW_MODESET, atomic_req).is_ok() { println!("Mode set"); } else { println!("No mode set"); }
    {
        ...
        let mut decoder = codec::context::Context::from_parameters(video.parameters())?.decoder().video()?;
        for (stream, packet) in context.packets() {
            if stream.index() == video_index {
                decoder.send_packet(&packet).unwrap();
                loop {
                    let mut video = Video::empty();
                    if decoder.receive_frame(&mut video).is_err() { break; }
                    ...
                    card.page_flip(crtc.handle(), fb, PageFlipFlags::empty(), None).unwrap();
                }
            }
        }
    }
